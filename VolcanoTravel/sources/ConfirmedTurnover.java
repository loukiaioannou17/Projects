...
public class ConfirmedTurnover extends javax.swing.JFrame {
    ...
    public ConfirmedTurnover(Connection con,CompanyManager cm,int id ) {
    ...
    private void initComponents(Connection con,CompanyManager cm,int id ) {
        ...
        jLabel1.setText("Confirmed Turnover");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt,Connection con,CompanyManager cm,int id ) {                                         
    	try{
    		CallableStatement s = con.prepareCall("{CALL ConfirmedTurnover(?)}");
    		s.setString(1, jTextField1.getText());
    		boolean flag = s.execute();
    		String result = "";
    		while (flag){
    			ResultSet rs = s.getResultSet();
    			result = result + "FlightMoney\tCarMoney\tRoomMoney\tTotal\n";
    			while (rs.next()){

...
public class ShowCustomer extends javax.swing.JFrame {
    ...
    public ShowCustomer(Connection con,Caller c,CompanyManager cm,int id) {
    ...
    private void initComponents(Connection con,Caller c,CompanyManager cm,int id) {
        ...
        jLabel2.setText("Give CustomerID");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt,Connection con,Caller c,CompanyManager cm,int id) {                                         
    	...
    	try{
    		...
    		ResultSet rs = s.executeQuery("SELECT * FROM Customer WHERE Customer.CustomerID='"+jTextField1.getText()+"'");
    		label = label +"ID\tName\tSurname\tAddress\tEmail\tPhoneNumber\n";
    		while(rs.next()){
    			label = label +rs.getInt(1) +"\t";
    			label = label +rs.getString(4) +"\t";
    			label = label +rs.getString(2) +"\t";
    			label = label +rs.getString(3) +"\t";
    			label = label +rs.getString(5) +"\t";
    			label = label +rs.getInt(6) +"\t";

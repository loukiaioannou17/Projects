...
public class FullRoomReport extends javax.swing.JFrame {
    ...
    public FullRoomReport(Connection con,Caller c,CompanyManager cm,int id) {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt,Connection con,Caller c,CompanyManager cm,int id) {                                         
    	try{
    		...
    		ResultSet s = stmt.executeQuery("SELECT Book_Room.RoomID,Type,DiscountPrice,Score FROM (Book_Room INNER JOIN HotelRooms ON Book_Room.RoomID=HotelRooms.RoomID) WHERE  ( [CheckIn] BETWEEN '"+jTextField1.getText()+"' AND '"+jTextField2.getText()+"') AND (Confirmed=1);");
